...
def render(
    ...
    logging.debug("Starting render function for violin_plot")
    logging.debug(f"Selected y-axis: {selected_y}")

    logging.debug("Filtering null values")
    data = data\
    ...
    logging.debug("Creating violin plot")
    try:
        chart = alt.Chart(
            data,
            ...
            selected_y,
            as_=[selected_y, 'density']
        ).mark_area(orient='horizontal').encode(
            y=alt.Y(f'{selected_y}:Q', title=selected_y, axis=alt.Axis(
                labelColor='white',
                domainOpacity=0,
                gridOpacity=0.2,
                domain=False,
            )),
            x=alt.X('density:Q', title='Density', axis=alt.Axis(
                labelColor='white',
                domainOpacity=0,
                gridOpacity=0,
                domain=False,
